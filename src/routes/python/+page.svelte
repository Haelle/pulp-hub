<script lang="ts">
	...
	import { getAllPythonPackages, type PythonPackageGroup } from '$lib/pulp';

	let filter = $state('');
	let groups = $state<PythonPackageGroup[]>([]);
	...
	async function load() {
		...
		try {
			...
			const map = new SvelteMap<string, PythonPackageGroup>();
...
<div class="mx-auto max-w-6xl p-6 space-y-6">
	<h1 class="text-2xl font-bold">PyPI Packages</h1>
	<CliHint>
		<code class="bg-muted px-1.5 py-0.5 rounded">GET /pulp/api/v3/content/python/packages/</code>
		— pulp-cli does not support listing cached Python packages, use the REST API directly.
	</CliHint>
		...
		{#if filtered.length > 0}
			<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
				{#each filtered as group (group.name)}
					<a
						href="/python/packages/{encodeURIComponent(group.name)}"
						class="block"
						data-testid="python-package-card"
					>
