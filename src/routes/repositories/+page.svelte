<script lang="ts">
	...
	import { getDistributions, type PulpPaginated, type ContainerDistribution } from '$lib/pulp';

	const limit = 20;
	let offset = $state(0);
	let filter = $state('');
	let distributions = $state<PulpPaginated<ContainerDistribution> | null>(null);
	...
	async function load() {
		...
		error = '';
		try {
			distributions = await getDistributions(limit, offset);
		} catch (e) {
